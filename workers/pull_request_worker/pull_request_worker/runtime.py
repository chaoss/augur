...
from pull_request_worker.worker import GithubPullRequestWorker
...
def main(augur_url, host, port):
    ...
    config = { 
            'id': 'com.augurlabs.core.github_pull_request_worker.{}'.format(worker_port),
    ...
    app.worker = GithubPullRequestWorker(config) # declares the worker that will be running on this server with specified config
