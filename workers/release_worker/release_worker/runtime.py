...
from release_worker.worker import ReleaseWorker
...
def create_server(app, gw):
    ...
    def augwop_task():
        ...
        if request.method == 'POST': #will post a task to be added to the queue
            ...
            app.gh_release_worker.task = request.json
    ...
    def augwop_config():
        ...
        """
        return app.gh_release_worker.config
...
def main(augur_url, host, port):
    ...
    worker_info = read_config('Workers', 'release_worker', None, None)
    ...
    config = { 
            "id": "com.augurlabs.core.release_worker.{}".format(worker_port),
    ...
    app.gh_release_worker = ReleaseWorker(config) # declares the worker that will be running on this server with specified config

    create_server(app, None)
    logging.info("Starting Flask App with pid: " + str(os.getpid()) + "...")
    app.run(debug=app.debug, host=host, port=worker_port)
    if app.gh_release_worker._child is not None:
        app.gh_release_worker._child.terminate()
    try:
