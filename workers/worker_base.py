...
class Worker():
    ...
    def get_table_values(self, cols, tables, where_clause=""):
        ...
        params = {'col_str': col_str, 'table_str': table_str}
        if where_clause != '':
    ...
    def init_oauths(self, platform="github"):
        ...
        for oauth in [{'oauth_id': 0, 'access_token': self.config[key_name]}] + json.loads(pd.read_sql(oauthSQL, self.helper_db, params=params).to_json(orient="records")):
            if platform == "github":
    ...
    def retrieve_tuple(self, key_values, tables):
        ...
        retrieveTupleSQL = s.sql.text("""
            SELECT * FROM :table_str WHERE :where_str
            """.format(table_str, where_str))
        params={'table_str': table_str, 'where_str': where_str}
        values = json.loads(pd.read_sql(retrieveTupleSQL, self.db, params=params).to_json(orient="records"))
        return values
