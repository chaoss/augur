...
class MessageInsightsWorker(Worker):
    def __init__(self, config={}):
        ...
        super().__init__(worker_type, config, given, models, data_tables, operations_tables, platform=None)
    ...
    def message_analysis_model(self, task, repo_id):
        ...
        self.msg_analysis_last = self.get_max_id('message_analysis', 'msg_analysis_id')
        self.logger.info(f'Last id in message_analysis table is: {self.msg_analysis_last - 1}') 
        self.begin_date = ''
        ...
        if not self.full_train:
            last_msg_id = self.get_table_values(cols=['msg_id'], tables=['message_analysis'], where_clause=f'where msg_analysis_id={self.msg_analysis_last-1}')
            last_msg_id = last_msg_id['msg_id'].iloc[0]
            self.begin_date = self.get_table_values(cols=['msg_timestamp'], tables=['message'], where_clause=f'where msg_id={last_msg_id}')
            self.run_id = self.get_max_id('message_analysis', 'worker_run_id')
            self.begin_date = self.begin_date['msg_timestamp'].iloc[0]
        ...
        if not df_message.empty:
            ...
            for row in df_message.itertuples(index=False):
                try:
                    ...
                    self.logger.info(f'Inserted data point {self.results_counter} with msg_id {row.msg_id}')
            ...
            # Refactor: Calculate deviation from mean for all metrics
            ...
            senti_anomaly_timestamps = anomaly_df['period']
            ...
            try:
                for date in senti_anomaly_timestamps:
            ...
            # send_insights(repo_id, insights)
    ...
    def send_insight(self, repo_id, insights):
        try:
            ...
            to_send = {
                    ...
                    'insight_begin_date': self.begin_date, # date from when insights are calculated
                    ...
                    'sentiment_anomaly_timestamps': insights[3] # anomaly timestamps from sentiment trend (if any)
                }
            requests.post('https://ejmoq97307.execute-api.us-east-1.amazonaws.com/dev/insight-event', json=to_send)
        
        except Exception as e:
            self.logger.info("Sending insight to Auggie failed: {}".format(e))
