...
----------------------------------------
When contributing a new metric, please reference this list in order to make sure you are putting the metric in the correct spot in the order, according to the list. With the amount of metrics we have already, and plan to add, this helps contributors both new and old maintain clarity and order when working with metrics. 

A few things to note:
* This order is followed across the project. Data source files, test files, server.py, AugurAPI.js, frontend cards, and other places that metrics appear in should adhere to this order for maximum uniformity.
* Data source files should include a header for every group, even if there are no functions in that data source that provide a metric in that group.
* If you are adding an Experimental metric, simply add it at the end of the correct sub-category. If you would like to add a new Experimental sub-category, please open a pull request with your proposed addition.
