...
Augur supports ```Python 3.7``` through ```Python 3.11``` on all platforms. ```Python 3.11``` is the recommended version.
```Python 3.12``` and above do not yet work because of machine learning worker dependencies.On OSX, you can create a ```Python 3.11``` environment, by running:
```bash
