      ...
      <div v-if="loaded" class="col-12">
        ...
        <div class="repo-link-holder">
          <table class="is-responsive">
            <thead
              class="
                repo-link-table repo-link-table-body repo-link-table-header
              "
            >
              <tr>
                <th v-on:click="sortTable('url')">
                  URL
                  <div
                    class="arrow"
                    v-if="'url' == sortColumn"
                ...
                <th v-on:click="sortTable('rg_name')">
                  Repo Group Name
                  <div
                    class="arrow"
                ...
                <th v-on:click="sortTable('description')">
                  Repo Description
                  <div
                    class="arrow"
                    v-if="'description' == sortColumn"
                    v-bind:class="ascending ? 'arrow_up' : 'arrow_down'"
                  ></div>
                </th>
                <th v-on:click="sortTable('repo_count')">
                  Repo Count for this Group
                  <div
                    class="arrow"
                    v-if="'repo_count' == sortColumn"
                    v-bind:class="ascending ? 'arrow_up' : 'arrow_down'"
                  ></div>
                </th>
                <th v-on:click="sortTable('commits_all_time')">
                  Total Commit Count
                  <div
                    class="arrow"
                    v-if="'commits_all_time' == sortColumn"
                    v-bind:class="ascending ? 'arrow_up' : 'arrow_down'"
                  ></div>
                </th>
                <th v-on:click="sortTable('issues_all_time')">
                  Total Issue Count
                  <div
                    class="arrow"
                    v-if="'issues_all_time' == sortColumn"
                    v-bind:class="ascending ? 'arrow_up' : 'arrow_down'"
                  ></div>
                </th>
                <th v-on:click="sortTable('repo_status')">
                  Status
                  <div
                    class="arrow"
                    v-if="'repo_status' == sortColumn"
                    v-bind:class="ascending ? 'arrow_up' : 'arrow_down'"
                  ></div>
                </th>
              </tr>
      ...
      window.AugurAPI.getRepos().then((data) => {
        ...
        window.AugurAPI.getRepoGroups().then((data) => {
          ...
          this.repo_groups.forEach((group) => {
            ...
            this.repo_relations[group.rg_name] = this.repos.filter(function (
              repo
            ) {
