...
<template>
  <d-button-group vertical>
    <d-button
      outline
      :active="tab == 'repo_overview'"
      class="tabSelectorButton"
      @click="onTab"
      value="repo_overview"
      >Overview</d-button
    >
    <d-button
      outline
      :active="tab == 'repo_risk'"
      class="tabSelectorButton"
      theme="secondary"
      @click="onTab"
      value="repo_risk"
      >Risk Metrics</d-button
    >
    <d-button
      v-if="getComparedReposNames.length > 0"
      outline
      :active="tab == 'repo_overview_compare'"
      class="tabSelectorButton"
      theme="info"
      @click="onTab"
      value="repo_overview_compare"
      >Comparison</d-button
    >
  </d-button-group>
</template>
...
@Options({
  ...
  computed: {
    ...mapGetters("common", ["tab"]),
    ...mapGetters("compare", ["base", "comparedRepos"]),
  },
})
export default class TabSelector extends Vue {
  ...
  onTab(e: any) {
    ...
    this.$router.push({
      name: e.target.value,
      params: { repo: ref, group: this.base.rg_name }, //, compares: names}
    });
  }
}
</script>
