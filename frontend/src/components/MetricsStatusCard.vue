      ...
      <div class="row gutters">
        ...
        <div class="col col-4">
          <label
            ...
            >
              <option
                v-for="data_source in metadata['data_sources']"
        ...
        <div class="col col-4">
          <label
            ...
            <select
              id="metric_type"
              @change="getMetricsStatus()"
              v-model="selected_metric_type"
            >
              <option
                v-for="metric_type in metadata['metric_types']"
                v-bind:value="metric_type"
              >
                {{ metric_type }}
              </option>
            </select>
        ...
        <div class="col col-4">
          <label
            >Backend Status:
            <select
              id="metric_backend_status"
              @change="getMetricsStatus()"
              v-model="selected_backend_status"
            >
        ...
        <div class="col col-4">
          <label
            >Frontend Status:
            <select
              id="metric_frontend_status"
              @change="getMetricsStatus()"
              v-model="selected_frontend_status"
            >
              <option value="all">all</option>
              <option value="unimplemented">unimplemented</option>
              <option value="implemented">implemented</option>
        ...
        <div class="col col-4">
          <label
            >Defined:
            <select
              id="metric_is_defined"
              @change="getMetricsStatus()"
              v-model="selected_is_defined"
            >
              <option value="all">all</option>
        ...
        <tbody class="metricsCardBody">
          <tr v-for="metric in metricsStatus">
            <div class="metricsCardDiv">
              <td
                v-bind:style="{ color: getBackendStatusColor(metric) }"
                class="metricsCardCellA"
              >
                {{ metric.backend_status }}
              </td>
              <td
                v-bind:style="{ color: getFrontendStatusColor(metric) }"
                class="metricsCardCellB"
              >
                {{ metric.frontend_status }}
              </td>

              <template v-if="metric.url != '/'">
                <td class="metricsCardCellC">
                  <a :href="metric.documentation_url">{{
                    metric.display_name
                  }}</a>
                </td>
              </template>
