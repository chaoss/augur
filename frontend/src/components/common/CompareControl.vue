        ...
        <d-row class="align-items-center">
          ...
          <d-col cols="12" lg="4" v-if="!isGroup">
            <multiselect
              ...
              :preserve-search="true"
              :options="RepoOptions"
              :disabled="getSelectedGroups.length === 0"
...
">keyboard_arrow_down</i></div>
        ...
        <d-row>
          <d-badge
            ...
            pill
            class="mx-2 mt-2"
            v-for="item in getSelectedRepos"
          >
            {{ item }}
          ...
          <d-badge
            ...
            pill
            class="mx-2 mt-2"
            v-for="item in getSelectedGroups"
          >
            {{ item }}
                ...
                <label>
                  ...
                  <d-row>
                    <d-col>
                      <d-form-checkbox
                        name="comparebaseline"
                        :checked="showBelowAverage"
                        @change="onShowBelowAverageChange"
                        inline
                        >Show users with below-average total contributions
                      </d-form-checkbox>
                      <sup class="warn"></sup>
                    </d-col>
                  </d-row>
                ...
                <d-row>
                  <d-col cols="12">
                    <div class="col col-12">
                      <small class="warn">
                        - These options affect performance</small
                      >
                    </div>
                    <div class="col col-11">
                      <small
                        >1. Line charts show a rolling mean over
                        {{ trailingAverage }} days with data points at each
                        {{ trailingAverage / 4 }}-day interval</small
                      >
                    </div>
                  </d-col>
                </d-row>
                    ...
                    <d-col>
                      ...
                      <d-input-group append="day average" class="mb-3">
                        <d-form-input
                          ...
                          @change="onTrailingAverageChange"
                        ></d-form-input>
                      </d-input-group>
                    </d-col>
                  </d-row>
...
@Options({
  ...
  computed: {
    ...mapGetters("common", ["repos", "repoGroups", "repoRelations"]),
    ...mapGetters("compare", [
      "base",
      "compare",
      "trailingAverage",
  ...
  methods: {
    ...mapMutations("common", ["mutateStartDate", "mutateEndDate"]),
    ...
    ...mapActions("common", ["loadRepoGroups", "loadRepos"]),
  },
})
export default class CompareControl extends Vue {
  ...
  onCompare(e: any) {
    ...
    if (!this.isGroup) {
      ...
      for (repo of this.selectedRepoEntities) {
        console.log(repo);
        if (i == 0) comparedRepoIds += String(repo.repo_id);
        else comparedRepoIds += "," + String(repo.repo_id);
        i++;
      }
      this.setComparedRepos({
        names: [this.selectedRepos.join(",")],
        ids: [comparedRepoIds],
      }).then(() => {
        router.push({
      ...
      router.push({
        ...
        params: {
          group: this.base.rg_name,
          repo_group_id: this.base.repo_group_id,
          compares: this.selectedGroups.join(","),
        },
      });
    }
  }
  ...
  onReset() {
    ...
    this.setVizOptions(initialState);
