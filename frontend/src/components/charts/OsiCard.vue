  ...
  methods: {
    list: function (indi) {
      ...
      let uriContent = URL.createObjectURL(
        new Blob([JSON.stringify(oList, null, 2)], {
      ...
      if (type === "reload") {
        if (indi == true) {
          link.setAttribute(
            "download",
            Object.keys(apiData)[0] + ".approved.licenses.json"
          );
        } else {
          link.setAttribute(
            "download",
            Object.keys(apiData)[0] + ".notapproved.licenses.json"
          );
        }
      } else {
        if (indi == true) {
          link.setAttribute(
            "download",
            Object.keys(apiData)[1] + ".approved.licenses.json"
          );
        } else {
          link.setAttribute(
            "download",
            Object.keys(apiData)[1] + ".notapproved.licenses.json"
          );
        }
      }
