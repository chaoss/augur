  ...
  <d-card>
    <d-card-body :title="title" class="text-center">
      ...
      <div v-if="loaded">
        ...
        <p>
          <strong
            >Click on a license count to download a list of associated
            files</strong
          >
        </p>
              ...
              <td>
                <template v-if="el['license_id'] < 500">
                  <a
                    ...
                    v-on:click="linfo(el['license_id'])"
                  >
                    {{ el["count"] }}
                  </a>
                </template>
                <template v-if="el['license_id'] >= 500">
                  <a
                    href="javascript:void(0)"
                    v-on:click="linfoF(el['license_id'])"
                  >
                    {{ el["count"] }}
                  </a>
                </template>
  ...
  methods: {
    ...
    linfoF: function () {
      ...
      let apiData = JSON.parse(
        JSON.stringify(this.$store.state.common.apiRepos)
      );
      ...
      fetch(`${host}/api/unstable/500/False/${groupID}/${repoID}/license-files`)
        ...
        .then((res) => {
          ...
          let uriContent = URL.createObjectURL(
            new Blob([JSON.stringify(res_refined, null, 2)], {
          ...
          if (type === "reload") {
            link.setAttribute(
              "download",
              Object.keys(apiData)[0] + ".NOASSERTION.files.json"
            );
          } else {
            link.setAttribute(
              "download",
              Object.keys(apiData)[1] + ".NOASSERTION.files.json"
            );
          }
    ...
    linfo: function (license_id) {
      ...
      console.log(repoID);
      fetch(
        `${host}/api/unstable/${license_id}/True/${groupID}/${repoID}/license-files`
      )
        .then((res) => res.json())
        .then((res) => {
          let res_refined: { [index: string]: number } = {};
          ...
          if (type === "reload") {
            link.setAttribute(
              "download",
              Object.keys(apiData)[0] + "." + res[0].short_name + ".files.json"
            );
          } else {
            link.setAttribute(
              "download",
              Object.keys(apiData)[1] + "." + res[1].short_name + ".files.json"
            );
          }
          let event = new MouseEvent("click");
          link.dispatchEvent(event);
