...
<template>
  <aside class="main-sidebar col-1 px-0 mainSideBaraside">
          ...
          <div class="d-table m-auto">
            <a href="/">
              <img
                src="@/assets/logo.png"
                id="logo"
                alt="CHAOSS: Community Health Analytics for Open Source Software"
              />
            </a>
            <!-- <span v-if="!hideLogoText" class="d-none d-md-inline ml-1">Augur Dashboard</span> -->
          </div>
        </a>
        <a class="toggle-sidebar d-lg-none" @click="handleToggleSidebar()">
          <!--d-sm-inline d-md-none d-lg-none-->
    ...
    <div class="nav-wrapper">
      <d-nav class="flex-column mainSideBarNav">
        <li
          ...
          @click="checkForRepoRouteChange($event)"
        >
          <d-link
            class="mainSideBarLink"
            :class="[
              'nav-link',
        ...
        <li v-if="'repo_id' in base" class="mainSideBarViF">
          <span
            >Different views
            <p class="mainSideBarSpan">for this repo:</p></span
          >
        </li>
        <li v-if="'repo_id' in base" class="mainSideBarVIF">
...
@Component({
  ...
  computed: {
    ...mapGetters("compare", ["base", "comparisonType", "comparisionSize"]),
  },
  methods: {
    ...mapMutations("compare", ["resetCompared"]),
  },
})
export default class MainSidebar extends Vue {
  ...
  checkForRepoRouteChange(e: any) {
    ...
    let currentlySelectedRepoGroup = Object.keys(
      this.$store.state.common.apiGroups
    )[Object.keys(this.$store.state.common.apiGroups).length - 1].split("/")[0];
