    ...
    <div class="row" style="transform: translateX(1rem)">
      <div class="row col col-8">
        <d-card v-if="!loadedInsights">
          <d-card-header class="border-bottom">
            <a
              href="#"
              class="underline text-fiord-blue"
              @click="onRepoOverview()"
            >
        ...
        <d-card
          v-if="loadedInsights"
          class="card-small"
          style="margin-bottom: 1rem; height: 16.5% !important"
        >
          <!-- Card Header -->
          <d-card-header class="border-bottom">
            <a
          ...
          <d-card-body
            ref="mainInsightContainer"
            class="pt-0 main-insight-container"
            style="margin-bottom: 126rem !important"
          >
            <!--  -->
            ...
            <main-insight
              ...
              color="black"
              source="main"
              :insight="insights[getGroup][getRepo][getApiMetric]"
            ></main-insight>
          </d-card-body>
        </d-card>
      </div>
      ...
      <div class="row col col-4" style="transform: translateX(3rem)">
        <div class="col-12 page-header row no-gutters py-4">
          <div class="col-12 text-center text-sm-left mb-0">
            <h3 class="page-title" style="font-size: 1rem">
              View insights for other repo groups
            </h3>
          </div>
        </div>
        ...
        <d-col
          ...
          v-for="(group, idx) in Object.keys(insights).slice(0, 5)"
          :key="idx"
          lg="12"
          sm="12"
          class="mb-4"
          ...
          <d-card class="card-small card">
            ...
            <div class="p-0 card-body">
              <div class="list-group-small list-group list-group-flush">
                <div
                  v-for="(repo, i) in Object.keys(insights[group]).slice(0, 5)"
                  class="d-flex px-3 list-group-item"
                  style="text-align: left"
                >
                  <a
                    href="#"
                    class="underline text-semibold text-fiord-blue"
                    style="width: 100%"
                    @click="
                      onInspectInsight(
                        insights[group][repo][
                          Object.keys(insights[group][repo]).slice(0, 1)[0]
                        ][0]
                      )
                    "
                  >
                    <span
                      class="text-semibold text-fiord-blue underline"
                      style="font-size: 1rem; padding: 0; max-width: 10rem"
                      >{{ getRepoName(repo) }}</span
                    >

                    <div
                      v-if="loadedInsights"
                      v-for="metric in Object.keys(insights[group][repo]).slice(
                        0,
                        1
                      )"
                      style="margin: 0 0 0 auto; float: right"
                    >
                      <spark-chart
                        :color="colors[idx]"
                        :title="
                          metric +
                          ' (' +
                          insights[group][repo][metric][0].ri_field +
                          ')'
                        "
                        :url="repo"
                        :data="insights[group][repo][metric]"
                        style="
                          max-height: 50px;
                          padding-bottom: 0px;
      ...
      <d-row
        class="col-12"
        style="transform: translateY(-128rem); padding: 0 !important"
      >
        <div class="col-12 page-header row no-gutters py-4">
          <div class="col-12 col-sm-4 text-center text-sm-left mb-0">
            <h3 class="page-title" style="font-size: 1rem">
              View other top insights
            </h3>
        ...
        <d-col
          v-if="loadedInsights"
          v-for="(group, idx) in Object.keys(insights).slice(0, 2)"
          :key="idx"
          lg="4"
          md="4"
          sm="8"
        ...
        >
          <d-card
            v-if="idx < 4"
            v-for="repo in Object.keys(insights[group]).slice(0, 1)"
            class="card-small card-post card-post--1"
          >
            <div
              class="card-post__image"
              v-for="metric in Object.keys(insights[group][repo]).slice(0, 1)"
            >
              <d-badge
                pill
                :class="['card-post__category', 'bg-' + themes[idx]]"
                >{{ group }}</d-badge
              >
              <insight-chart
                style=""
            ...
            >
              <h5 class="card-title">
                <a
                  href="#"
                  @click="onGitRepo(insights[group][repo][metric])"
                  class="text-fiord-blue underline"
                  >{{ repo.substr(19) }}</a
                >
              </h5>
              <p
                class="card-text d-inline-block mb-1"
                style="font-size: 0.75rem"
              >
                This repository had
                {{ getPhrase(insights[group][repo][metric]) }}
              </p>
              <d-row>
                <d-col cols="12" sm="5"
                ...
                <d-col
                  cols="12"
                  sm="7"
                  style="transform: translateX(-1rem) !important"
                >
                  <d-button
                    size="sm"
                    @click="onInspectInsight(insights[group][repo][metric][0])"
...
export default class InspectInsight extends Vue {
  ...
  created() {
    ...
    this.loadRepoGroups().then((groups: any) => {
      ...
      Promise.all(addingGroupPromises).then((groups) => {
        ...
        this.endpoint({
          repoGroups: relevantApiGroups,
          endpoints: ["topInsights"],
        }).then((tuples: any) => {
          groups.forEach((group) => {
            if ("topInsights" in tuples[group.rg_name].groupEndpoints) {
              tuples[group.rg_name].groupEndpoints.topInsights.forEach(
                (tuple: any) => {
  ...
  getDirection(values: any[]) {
    ...
    if (values[i + 1]) {
      if (values[i + 1].value > values[i].value) return "arrow_upward";
