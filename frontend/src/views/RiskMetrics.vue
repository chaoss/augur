...
import Licenses from "../components/Licenses.ts";
...
export default class RiskMetrics extends Vue {
  ...
  created() {
    ...
    this.endpoint({ endpoints: this.sbom_endpoint, repos: [this.base] }).then(
      (tuples: any) => {
    ...
    this.endpoint({ endpoints: this.cii_endpoint, repos: [this.base] }).then(
      (tuples: any) => {
        Object.keys(tuples[ref]).forEach((endpoint) => {
          this.values[endpoint] = tuples[ref][endpoint];
