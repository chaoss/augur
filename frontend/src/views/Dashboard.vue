  ...
  <d-container fluid class="main-content-container px-4">
    ...
    <div class="page-header row no-gutters py-4">
      <div class="col-12 col-sm-4 text-center text-sm-left mb-0">
        ...
        <h3 class="dashboardHeader page-title">
          Most Anomalous Insights Across Your Repos
        </h3>
      </div>
    </div>
    ...
    <d-row>
      <d-col
        v-for="(record, idx) in highest_frame"
        :key="idx"
        lg="4"
        md="4"
        sm="6"
        ...
        >
          ...
          <div class="card-post__image" v-if="loadedInsights">
            <d-badge
              ...
              :class="[
                'card-post__category',
                'bg-' + (color_mapping[highest[idx].ri_metric].theme || 'info'),
              ]"
              >{{ highest[idx].ri_metric }} ({{
                highest[idx].ri_field
              }})</d-badge
            >
            <insight-chart
              style=""
              :data="
                insights[highest[idx].rg_name][highest[idx].repo_git][
                  highest[idx].ri_metric
                ]
              "
              :url="highest[idx].repo_git"
              :color="color_mapping[highest[idx].ri_metric].hex || '#FFC107'"
            ></insight-chart>
          ...
          <d-card-body v-if="loadedInsights">
            <h5 class="card-title">
              <a
                :id="idx"
                href="#"
                @click="onGitRepo(highest[idx])"
                class="text-fiord-blue underline"
                >{{ highest[idx].repo_git.substr(19) }}</a
              >
              <d-tooltip
                :target="'#' + idx"
...
export default class Dashboard extends Vue {
  ...
  timeframes: any = {};
  test: any[] = [
    "https://github.com/rails/ruby-coffee-script.git",
    "https://github.com/Comcast/Hygieia.git",
    "https://github.com/apache/jclouds-site.git",
    "https://github.com/apache/karaf-jclouds.git",
  ...
  created() {
    ...
    this.loadRepoGroups()
      .then((groups: any) => {
        ...
        Promise.all(addingGroupPromises)
          .then((groups: any) => {
            this.highest_frame = [];
            let dupesAllowed = this.desiredTopInsights - groups.length;
            let count = 0;
            groups.forEach((group: any) => {
              if (count < 12) this.highest_frame.push({});
            ...
            this.endpoint({
              ...
              .then((tuples: any) => {
                groups.forEach((group: any) => {
                  console.log(
                    "Group tuples: ",
                    tuples[group.rg_name].groupEndpoints.topInsights
                  );
                  if ("topInsights" in tuples[group.rg_name].groupEndpoints) {
                    ...
                    for (
                      n = 0;
                      n <
                      tuples[group.rg_name].groupEndpoints.topInsights.length;
                      n++
                    ) {
                      // tuples[group.rg_name].groupEndpoints.topInsights.forEach((tuple:any) => {

                      let tuple =
                        tuples[group.rg_name].groupEndpoints.topInsights[n];
                      ...
                      this.highest.forEach((record: any) => {
                        ...
                        if (
                          record.repo_git == tuple.repo_git ||
                          record.repo_id == tuple.repo_id
                        )
                      ...
                      for (let i = 0; i < this.highest.length; i++) {
                        if (
                          tuple.date > this.highest[i].date &&
                          !alreadyIncluded &&
                          this.highest.length >= this.desiredTopInsights
                        ) {
                          if (
                            tuples[group.rg_name].groupEndpoints.topInsights[
                              n + 1
                            ...
                            if (
                              tuples[group.rg_name].groupEndpoints.topInsights[
                                n + 1
                              ].repo_git == tuple.repo_git ||
                              tuples[group.rg_name].groupEndpoints.topInsights[
                                n - 1
                              ].repo_git == tuple.repo_git
                            ) {
                              console.log(
                                "Update hightest condition met: ",
                                tuple
                              );
                ...
                console.log(
                  "check:",
                  this.insights,
                  JSON.stringify(this.highest[0])
                );
                this.loadedInsights = true;
              })
              .catch((e: any) => {
                console.log(
                  "Error occurred top insights for all repo groups: ",
                  e
                );
              });
          })
      ...
      .catch((e: any) => {
        console.log(
          "Error occurred loading repo groups on Dashboard level: ",
          e
        );
      });
