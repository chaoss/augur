...
def test_annual_commit_count_ranked_by_new_repo_in_repo_group_by_repo():
...
def test_annual_commit_count_ranked_by_new_repo_in_repo_group_by_group():
    response = requests.get('http://localhost:5000/api/unstable/repo-groups/10/annual-commit-count-ranked-by-new-repo-in-repo-group')
    data = response.json()
    assert response.status_code == 200
    assert len(data) >= 1
...
def test_annual_commit_count_ranked_by_repo_in_repo_group_by_repo():
...
def test_annual_commit_count_ranked_by_repo_in_repo_group_by_group():
...
def test_top_committers_by_repo():
...
def test_top_committers_by_group():
