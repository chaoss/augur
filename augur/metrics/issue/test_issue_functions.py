...
def test_issues_new(metrics):
    ...
    #begin_date & end_date
    assert metrics.issues_new(24, 21979, period='week', begin_date='2017',
                               end_date='2017-05').iloc[1]['issues'] > 0
    assert metrics.issues_new(24, period='month', begin_date='2017-05',
...
def test_issue_duration(metrics):
    # repo
    assert metrics.issue_duration(24, 21681).iloc[0]['duration'] == '0 days 01:06:31.000000000'

    # repo_group
    assert metrics.issue_duration(24).iloc[3]['duration'] == '5 days 05:18:21.000000000'
...
def test_issues_first_time_closed(metrics):
    ...
    # repo id
    assert metrics.issues_first_time_closed(21, repo_id=21222, period='year').isin(
    ...
    assert metrics.issues_first_time_closed(21, repo_id=21222, period='year', begin_date='2019-1-1 00:00:00',
                                             end_date='2019-12-31 23:59:59').isin([pd.Timestamp('2019-01-01 00:00:00', tz='UTC')]).any().any()
...
def test_average_issue_resolution_time(metrics):
    #repo
    assert metrics.average_issue_resolution_time(24, 21464).isin(
        ['maven-release', '276 days 13:54:13.2']).any().any()

    # repo_group
    assert metrics.average_issue_resolution_time(24).isin(
        ['maven-release', '276 days 13:54:13.2']).any().any()
...
def test_issue_comments_mean(metrics):
    ...
    assert metrics.issue_comments_mean(23, 21353).any().any()
    assert metrics.issue_comments_mean(23, group_by='year').any().any()
    assert metrics.issue_comments_mean(23, 21353, group_by='year').any().any()

def test_issue_comments_mean_std(metrics):
    assert metrics.issue_comments_mean_std(23).any().any()
    assert metrics.issue_comments_mean_std(23, 21353).any().any()
    assert metrics.issue_comments_mean_std(23, group_by='year').any().any()
    assert metrics.issue_comments_mean_std(23, 21353, group_by='year').any().any()