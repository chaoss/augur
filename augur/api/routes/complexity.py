    ...
    def get_project_languages():
        project_languages_sql = s.sql.text("""
            SELECT
                ...
                ORDER BY e.repo_id
        """)
        results = pd.read_sql(project_languages_sql,  server.engine.connect())
    ...
    def get_project_files():
        project_files_sql = s.sql.text("""
            SELECT
                ...
                ORDER BY e.repo_id
        """)
        results = pd.read_sql(project_files_sql,  server.engine.connect())
        data = results.to_json(orient="records", date_format='iso', date_unit='ms')
        return Response(response=data,
                    status=200,
                    mimetype="application/json")
    ...
    def get_project_lines():
        project_lines_sql = s.sql.text("""
            SELECT
                ...
                ORDER BY e.repo_id   
        """)
        results = pd.read_sql(project_lines_sql,  server.engine.connect())
        data = results.to_json(orient="records", date_format='iso', date_unit='ms')
        return Response(response=data,
                    status=200,
                    mimetype="application/json")
    ...
    def get_project_comment_lines():
        comment_lines_sql = s.sql.text("""
            SELECT
                ...
                ORDER BY e.repo_id
        """)
        results = pd.read_sql(comment_lines_sql,  server.engine.connect())
        data = results.to_json(orient="records", date_format='iso', date_unit='ms')
        return Response(response=data,
                    status=200,
                    mimetype="application/json")
    ...
    def get_project_blank_lines():
        blank_lines_sql = s.sql.text("""
            SELECT
                ...
                ORDER BY e.repo_id
            """)
        results = pd.read_sql(blank_lines_sql,  server.engine.connect())
        data = results.to_json(orient="records", date_format='iso', date_unit='ms')
        return Response(response=data,
                    status=200,
                    mimetype="application/json")
    ...
    def get_project_file_complexity():
        project_file_complexity_sql = s.sql.text("""
            SELECT
                ...
                ORDER BY e.repo_id
            """)
        results = pd.read_sql(project_file_complexity_sql,  server.engine.connect())
        data = results.to_json(orient="records", date_format='iso', date_unit='ms')
        return Response(response=data,
                    status=200,
                    mimetype="application/json")
