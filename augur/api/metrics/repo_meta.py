...
def code_changes(repo_group_id, repo_id=None, period='week', begin_date=None, end_date=None):
    ...
    if not repo_id:
        ...
        results = pd.read_sql(code_changes_SQL, engine.connect(), params={'repo_group_id': repo_group_id, 'period': period,
        ...
        results = pd.read_sql(code_changes_SQL, engine.connect(), params={'repo_id': repo_id, 'period': period,
                                                                    'begin_date': begin_date, 'end_date': end_date})
...
def code_changes_lines(repo_group_id, repo_id=None, period='day', begin_date=None, end_date=None):
    ...
    if not repo_id:
        ...
        results = pd.read_sql(code_changes_lines_SQL, engine.connect(), params={'repo_group_id': repo_group_id, 'period': period,
                                                                    'begin_date': begin_date, 'end_date': end_date})
        ...
        results = pd.read_sql(code_changes_lines_SQL, engine.connect(), params={'repo_id': repo_id, 'period': period,
                                                                        'begin_date': begin_date, 'end_date': end_date})
        return results
...
def sub_projects(repo_group_id, repo_id=None, begin_date=None, end_date=None):
    ...
    if not end_date:
        ...
        if repo_id:
            ...
            results = pd.read_sql(sub_projectsSQL, engine.connect(), params={'repo_id': repo_id,
                                                                    'begin_date': begin_date, 'end_date': end_date})
        else:
            sub_projectsSQL = s.sql.text("""
                SELECT COUNT(*) AS sub_project_count
            ...
            results = pd.read_sql(sub_projectsSQL, engine.connect(), params={'repo_group_id': repo_group_id,
                                                                    'begin_date': begin_date, 'end_date': end_date})
    return results
...
def sbom_download(repo_group_id, repo_id=None):
    ...
    return pd.read_sql(dosocs_SQL, engine.connect(), params=params)
...
def cii_best_practices_badge(repo_group_id, repo_id=None):
    ...
    raw_df = pd.read_sql(cii_best_practices_badge_SQL, engine.connect(), params={'repo_id': repo_id})
...
def forks(repo_group_id, repo_id=None):
    ...
    if not repo_id:
        ...
        results = pd.read_sql(forks_SQL, engine.connect(), params={'repo_group_id': repo_group_id})
        return results
        ...
        results = pd.read_sql(forks_SQL, engine.connect(), params={'repo_id': repo_id})
        return results
...
def fork_count(repo_group_id, repo_id=None):
    ...
    if not repo_id:
        ...
        results = pd.read_sql(fork_count_SQL, engine.connect(), params={'repo_group_id': repo_group_id})
        return results
    else:
        fork_count_SQL = s.sql.text("""
        ...
        results = pd.read_sql(fork_count_SQL, engine.connect(), params={'repo_id': repo_id})
        return results
...
def languages(repo_group_id, repo_id=None):
    ...
    if not repo_id:
        ...
        results = pd.read_sql(languages_SQL, engine.connect(), params={'repo_group_id': repo_group_id})
        return results
        ...
        results = pd.read_sql(languages_SQL, engine.connect(), params={'repo_id': repo_id})
        return results
...
def license_files(license_id, spdx_binary, repo_group_id, repo_id=None,):
        ...
        results = pd.read_sql(license_data_SQL, engine.connect(), params={'repo_id': repo_id, 'spdx_binary': spdx_binary, 'license_id': license_id})
        return results
...
def license_declared(repo_group_id, repo_id=None):
    ...
    results = pd.read_sql(license_declared_SQL, engine.connect(), params={'repo_id': repo_id})
    return results
...
def license_coverage(repo_group_id, repo_id=None):
    ...
    results = pd.read_sql(license_declared_SQL, engine.connect(), params={'repo_id': repo_id, 'repo_group_id':repo_group_id})
...
def stars(repo_group_id, repo_id=None):
    ...
    if not repo_id:
        ...
        results = pd.read_sql(stars_SQL, engine.connect(), params={'repo_group_id': repo_group_id})
        return results
    ...
    results = pd.read_sql(stars_SQL, engine.connect(), params={'repo_id': repo_id})
    return results
...
def stars_count(repo_group_id, repo_id=None):
    ...
    if not repo_id:
        ...
        results = pd.read_sql(stars_count_SQL, engine.connect(), params={'repo_group_id': repo_group_id})
        return results
    else:
        stars_count_SQL = s.sql.text("""
        ...
        results = pd.read_sql(stars_count_SQL, engine.connect(), params={'repo_id': repo_id})
        return results
...
def watchers(repo_group_id, repo_id=None):
    ...
    if not repo_id:
        ...
        results = pd.read_sql(watchers_SQL, engine.connect(), params={'repo_group_id': repo_group_id})
        return results
        ...
        results = pd.read_sql(watchers_SQL, engine.connect(), params={'repo_id': repo_id})
        return results
...
def watchers_count(repo_group_id, repo_id=None):
    ...
    if not repo_id:
        ...
        results = pd.read_sql(watchers_count_SQL, engine.connect(), params={'repo_group_id': repo_group_id})
        return results
    else:
        watchers_count_SQL = s.sql.text("""
        ...
        results = pd.read_sql(watchers_count_SQL, engine.connect(), params={'repo_id': repo_id})
        return results
...
calendar year (a new repo in that year), show all commits for that year (total for year by repo).
    ...
    results = pd.read_sql(cdRgNewrepRankedCommitsSQL, engine.connect(), params={ "repo_group_id": repo_group_id,
...
def lines_of_code_commit_counts_by_calendar_year_grouped(repo_url, calendar_year=None, interval=None):
    ...
    results = pd.read_sql(cdRepTpIntervalLocCommitsSQL, engine.connect(), params={"repourl": '%{}%'.format(repo_url), 'calendar_year': calendar_year})
    return results
...
def average_weekly_commits(repo_group_id=None, repo_id=None, calendar_year=None):
    ...
    results = pd.read_sql(average_weekly_commits_sql, engine.connect(), params={"repo_group_id": repo_group_id,
            "repo_id": repo_id, "calendar_year": calendar_year})
    return results
...
def aggregate_summary(repo_group_id, repo_id=None, begin_date=None, end_date=None):
    ...
    if not repo_id:
        ...
        results = pd.read_sql(summarySQL, engine.connect(), params={'repo_group_id': repo_group_id,
                                                        'begin_date': begin_date, 'end_date': end_date})
        return results
    else:
        summarySQL = s.sql.text("""
        ...
        results = pd.read_sql(summarySQL, engine.connect(), params={'repo_id': repo_id,
                                                        'begin_date': begin_date, 'end_date': end_date})
        return results
