...
@register_metric()
def release_frequency(repo_group_id, repo_id=None, period='day', begin_date=None, end_date=None):
    """ Returns a timeseries of releases occurring within a specific period
    ...
    if not repo_id:
        if period == 'year':
            releases_SQL = s.sql.text("""
                SELECT
                    repo_id,
                    COUNT(release_published_at) / ((DATE_PART('day', AGE(:end_date, :begin_date))/365) +
            ...
            releases_SQL = s.sql.text("""
                SELECT
                    repo_id,
                    COUNT(release_published_at) / ((DATE_PART('day', AGE(:end_date, :begin_date))/30) +
                        DATE_PART('month', AGE(:end_date, :begin_date)) +
                        (DATE_PART('year', AGE(:end_date, :begin_date))*12)) AS releases_over_time
                FROM
                    releases
                WHERE release_published_at BETWEEN :begin_date AND :end_date
                AND repo_id IN (SELECT repo_id FROM repo WHERE repo_group_id=:repo_group_id)
            ...
            releases_SQL = s.sql.text("""
                SELECT
                    repo_id,
                    COUNT(release_published_at) / (DATE_PART('day', AGE(:end_date, :begin_date)) +
                        (DATE_PART('month', AGE(:end_date, :begin_date))*30) +
                        (DATE_PART('year', AGE(:end_date, :begin_date))*365)) AS releases_over_time
                FROM
                    releases
                WHERE release_published_at BETWEEN :begin_date AND :end_date
                AND repo_id IN (SELECT repo_id FROM repo WHERE repo_group_id=:repo_group_id)
