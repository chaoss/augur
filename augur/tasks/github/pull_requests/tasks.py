...
def collect_pull_request_review_comments(repo_git: str) -> None:
    ...
    with GithubTaskSession(logger, engine) as session:
        ...
        query = session.query(PullRequestReview).filter(PullRequestReview.repo_id == repo_id)
        ...
        pr_review_messages = GithubPaginator(review_msg_url, session.oauths, logger)
...
def collect_pull_request_reviews(repo_git: str) -> None:
    ...
    with GithubTaskSession(logger, engine) as session:
        ...
        for index, pr in enumerate(prs):
            ...
            pr_reviews = GithubPaginator(pr_review_url, session.oauths, logger)
        ...
        session.insert_data(pr_reviews, PullRequestReview, pr_review_natural_keys)
