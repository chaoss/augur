...
class GithubApiKeyHandler():
    ...
    @staticmethod
    def get_rate_limit_remaining(httpx_client, github_api_key: str) -> int:

        headers = {'Authorization': f'token {github_api_key}'}

        data = httpx_client.request(method="GET", url=RATE_LIMIT_URL, headers=headers, timeout=180).json()
