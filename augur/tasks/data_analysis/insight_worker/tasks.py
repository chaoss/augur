...
def clear_insights(repo_id, new_endpoint, new_field, logger):
    ...
    deleteSQL = s.sql.text("""
        DELETE 
            ...
            WHERE
                repo_id = :repo_id
                AND ri_metric = :new_endpoint
                AND ri_field = :new_field
    """)
    try:
        with engine.connect() as conn:
            result = conn.execute(deleteSQL, {"repo_id": repo_id, "new_endpoint": new_endpoint, "new_field": new_field})
    except Exception as e:
...
def clear_insight(repo_id, new_score, new_metric, new_field, logger):
    ...
    """)
    with engine.connect() as conn:
        rec = json.loads(pd.read_sql(recordSQL, conn, params={"repo_id": repo_id, "new_metric": new_metric, "new_field": new_field}).to_json(orient='records'))
