...
def test_issue_with_comments_by_group(metrics):
    response = requests.get('http://localhost:5000/api/unstable/repo-groups/10/repos/25430/issue-with-comments')
    data = response.json()
    assert response.status_code == 200
    assert len(data) >= 1

def test_issue_with_comments_by_repo(metrics):
    response = requests.get('http://localhost:5000/api/unstable/repos/25430/issue-with-comments')
    data = response.json()
    assert response.status_code == 200
    assert len(data) >= 1
