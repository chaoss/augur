...
For this sprint we continued to investigate the issues, this time with the help of Professor Goggins. For our final sprint, we had to move the goalposts a bit as is very common in the SDLC. As it turns out, the error was much deeper and more systemic than it seemed from the outside, and we were unable to locate the exact source of the errors with the backend causing the `blocked loading mixed active content` errors. We did however add to the documentation by adding an "Enabling HTTPS" section to the `deployment/nginx-configuration.rst` section which overviewed what we were able to get working up to this point. Additionally since it would cause problems in the production version of Augur we rolled back our changes from Sprint 3 so we could merge our documentation with the official branch of Augur.
